...
from motor.generador_nl import generar_nl, _llamar_groq, MockGroqNLBackend
...
class TestGeneradorNL(unittest.TestCase):
    ...
    def test_sin_api_key_usa_plantilla(self):
        """No GROQ_API_KEY → fuente='plantilla'."""
    ...
    def test_mock_backend_devuelve_texto_mock(self):
        """Mock backend custom text should be returned with fuente='groq'."""
        mock = MockGroqNLBackend()
        mock.response_text = "Texto de prueba especial."
        intent = _make_intent()
        dato = _make_dato()

        # Inject mock via monkeypatching _llamar_groq
        import motor.generador_nl as gnl
        original_llamar = gnl._llamar_groq

        def mock_llamar(*args, **kwargs):
            kwargs["_backend"] = mock
            return original_llamar(*args, **kwargs)

        gnl._llamar_groq = mock_llamar
        ...
        try:
            ...
            self.assertEqual(fuente, "groq")
            self.assertEqual(texto, "Texto de prueba especial.")
        finally:
            gnl._llamar_groq = original_llamar
    ...
    def test_mock_backend_error_usa_fallback(self):
        ...
        gnl._llamar_groq = mock_llamar_raise
        original_key = os.environ.get("GROQ_API_KEY")
        os.environ["GROQ_API_KEY"] = "fake-key-for-test"
        try:
            texto, fuente = generar_nl(
